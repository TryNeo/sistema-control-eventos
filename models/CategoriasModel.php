...
class CategoriasModel extends Mysql
{
    ...
    {
        $sql = "SELECT id_categoria,nombre_categoria,descripcion,estado FROM categoria_evento where estado=1 ORDER BY id_categoria DESC";
    ...
    {
        $sql = "SELECT id_categoria,nombre_categoria,descripcion,estado FROM categoria_evento WHERE estado =1 ";
        $request = $this->select_sql_all($sql);
        return $request;
    }
    ...
    public function selectCategoria(int $id_categoria){
        ...
        $sql = "SELECT id_categoria,nombre_categoria,descripcion,estado FROM categoria_evento where id_categoria =$this->intCat";
        $request = $this->select_sql($sql);
        return $request;

    }
    public function insertCategoria(string $categoriaInput, string $descriInput)
    {
        $return = "";
        ...
        $request = $this->select_sql_all($sql);
        if (empty($request)) {
            $sql_insert = "INSERT INTO categoria_evento(nombre_categoria,descripcion,estado,fecha_crea) values (?,?,1,now())";
    ...
    public function updateCategoria(int $intCategoria,string $categoriaInput,string $descriInput)
    {
        ...
        if (empty($request_update)){
            $sql_udpate = "UPDATE categoria_evento SET nombre_categoria = ?, descripcion = ?,fecha_modifica = now()  WHERE id_categoria = $this->intCat";
            $data = array($this->strCat,$this->strDescrip);

...
class InvitadosModel extends Mysql
{
    ...
    {
        $sql = "SELECT id_invitado,nombre_invitado,apellido_invitado,descripcion,url_imagen,estado FROM invitados WHERE estado =1 ";
        $request = $this->select_sql_all($sql);
        return $request;
    }
    ...
    public function selectInvitado(int $id_invitado){
        ...
        $sql = "SELECT id_invitado,nombre_innvitado,apellido_invitado,descripcion,url_imagen,estado FROM invitados where id_invitado =$this->intInv";
        $request = $this->select_sql($sql);
        return $request;

    }

    public function insertInvitado(string $nombreInput, string $apellidoInput, string $descriInput, string  $str_imagen)
    {
        $return = "";
        $this->strNombre = $nombreInput;
        $this->strApellido = $apellidoInput;
        $this->strDescrip = $descriInput;
        $this->strImagen = $str_imagen;
    ...
    public function updateInvitado(int $intInvitado,string $nombreInput,string $apellidoInput, string $descriInput, string $str_imagen)
    {
        ...
        if (empty($request_update)){
            $sql_udpate = "UPDATE invitados SET nombre_invitado = ?, apellido_invitado = ?,descripcion = ?,fecha_modifica = now()  WHERE id_invitado = $this->intInv";
            $data = array($this->strNombre,$this->strApellido,$this->strDescrip,$this->strImagen);
